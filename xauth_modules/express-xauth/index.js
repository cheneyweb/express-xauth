        ...
        try {
            ...
            if (tokenVerify) {
                ...
                if (roleRegArr && roleRegArr instanceof Array && roleRegArr.length > 0) {
                    ...
                    // 失败：所有路由规则循环完毕均不能匹配
                    res.status(401).send({ err: true, res: `角色：[${tokenVerify.role}]未拥有访问权限` })
                }
                // 失败：角色拥有路由规则为空
                else {
                    res.status(401).send({ err: true, res: `角色：[${tokenVerify.role}]未配置访问权限` })
                }
            }
            // 失败：TOKEN解析失败
            else {
                res.status(401).send({ err: true, res: authConfig.errMsg })
            }
